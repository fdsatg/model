...
def save_model(
    ...
    checkpoint_paths = [
        output_dir / (f"checkpoint-{model_name}-{ratio}-{epoch_name}.pth")
    ]
...
def auto_load_model(
    ...
    if args.auto_resume and len(args.resume) == 0:
        ...
        all_checkpoints = glob.glob(
            os.path.join(output_dir, f"checkpoint-{model_name}-{ratio}-*.pth")
        )
        latest_ckpt = -1
        ...
        if latest_ckpt >= 0:
            args.resume = os.path.join(
                output_dir, f"checkpoint-{model_name}-{ratio}-{latest_ckpt}.pth"
            )

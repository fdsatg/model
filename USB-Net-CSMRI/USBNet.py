...
class IterationModule(nn.Module):
    ...
    def forward(self, mask, xk, dk, bk, x_init):
        ##########################STEP 1############################
        xk_k = torch.fft.fft2(xk) 
        mask_xk_k = xk_k * mask.view(1, 1, *(mask.shape))
        xk_t1 = torch.real(torch.fft.ifft2(mask_xk_k))

        xk = xk - self.step * xk_t1
        xk = xk + self.step * x_init

        xk_t2 = self.unit1(xk)
        xk_t2 = self.unit2(xk_t2 - (dk - bk))
        xk = xk - self.step * self.mu * xk_t2
...
def usbnet(ratio, pretrained=False, mask_type='Cartesian', **kwargs):
    ...
    if pretrained:
        checkpoint = load_pretrained_model(ratio, sys._getframe().f_code.co_name, mask_type)

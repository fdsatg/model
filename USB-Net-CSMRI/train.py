...
from utils.engine_finetune import train_one_epoch, evaluate
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='usbnet', type=str, metavar='MODEL',
                        help='Name of model to train')
    parser.add_argument('--mask_type', default='Cartesian', type=str, metavar='mask',
                        help='Name of mask')
...
def main(args):
    ...
    dataset_train = build_fastmridataset(is_train=True, args=args)
    if args.disable_eval:
        args.dist_eval = False
        dataset_val = None
    else:
        dataset_val = build_fastmridataset(is_train=False, args=args)
    ...
    checkpoint = torch.load("../USB-Net-ICS/model/checkpoint-usbnet-25-best.pth")
    ...
    utils.auto_load_model(
        ...
        optimizer=optimizer, loss_scaler=loss_scaler, model_name=args.model, ratio=args.cs_ratio, model_ema=model_ema)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and args.save_ckpt:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                utils.save_model(
                    ...
                    loss_scaler=loss_scaler, epoch=epoch, model_name=args.model, ratio=args.cs_ratio, model_ema=model_ema)
        if data_loader_val is not None:
            test_stats = evaluate(data_loader_val, model,
                                  device, use_amp=args.use_amp, args=args)
            print(
            ...
            if max_psnr < test_stats["psnr"]:
                ...
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="best", model_name=args.model, ratio=args.cs_ratio, model_ema=model_ema)
            print(f'Max PSNR: {max_psnr:.4f}')

            if log_writer is not None:
                log_writer.update(
                    test_acc1=test_stats['psnr'], head="perf", step=epoch)
                log_writer.update(
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema = evaluate(
                    data_loader_val, model_ema.ema, device, use_amp=args.use_amp, args=args)
                print(
                ...
                if max_psnr_ema < test_stats_ema["psnr"]:
                    ...
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch="best-ema", model_name=args.model, ratio=args.cs_ratio, model_ema=model_ema)
        ...
        if args.output_dir and utils.is_main_process():
            if log_writer is not None:
                log_writer.flush()
            with open(os.path.join(args.output_dir, f"{args.model}_{args.cs_ratio}_log.txt"), mode="a", encoding="utf-8") as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'fine-tuning', parents=[get_args_parser()])
